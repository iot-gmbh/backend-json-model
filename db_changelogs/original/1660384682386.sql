...
CREATE OR REPLACE FUNCTION public.get_durations(p_username character varying, p_date_from timestamp with time zone, p_date_until timestamp with time zone)
 RETURNS TABLE(id character varying, tenant character varying, parent_id character varying, title character varying, datefrom timestamp with time zone, dateuntil timestamp with time zone, totalduration numeric)
 LANGUAGE plpgsql
AS $function$ 
