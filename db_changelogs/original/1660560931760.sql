...
alter table "public"."iot_planner_categories" add column "validfrom" timestamp with time zone not null;

alter table "public"."iot_planner_categories" add column "validto" timestamp with time zone;

CREATE UNIQUE INDEX iot_planner_categories_pkey ON public.iot_planner_categories USING btree (id, validfrom);
...
create or replace view "public"."adminservice_categories" as  SELECT categories_0.id,
  ...
  WHERE ((categories_0.validfrom < now()) AND (categories_0.validto > now()));
...
create or replace view "public"."analyticsservice_categories" as  SELECT cat_0.id,
  ...
  WHERE ((wi_1.deleted IS NULL) AND ((cat_0.validfrom < now()) AND (cat_0.validto > now())));
