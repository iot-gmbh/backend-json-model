...
if (parseInt(version[0]) <= 1 && parseInt(version[1]) < 78) {
  ...
  sap.ui.require(["sap/ui/fl/FakeLrepConnector"], (FakeLrepConnector) => {
    jQuery.extend(FakeLrepConnector.prototype, {
      create(oChange) {
        return Promise.resolve();
      },
      stringToAscii(sCodeAsString) {
        if (!sCodeAsString || sCodeAsString.length === 0) {
          return "";
        }
        let sAsciiString = "";
        for (let i = 0; i < sCodeAsString.length; i++) {
      ...
      loadChanges() {
        ...
        return new Promise((resolve, reject) => {
          ...
          $.ajax({
            ...
            .then((oCachebusterContent) => {
              // we are looking for only change files
              const aChangeFilesPaths = Object.keys(oCachebusterContent).filter(
                (sPath) => sPath.endsWith(".change"),
              );
              $.each(aChangeFilesPaths, (index, sFilePath) => {
                ...
                if (sFilePath.indexOf("changes") === 0) {
                  ...
                  aPromises.push(
                    $.ajax({
                      ...
                      type: "GET",
                      cache: false,
                    }).then((sChangeContent) => JSON.parse(sChangeContent)),
                  );
                }
              });
            })
            .always(() => Promise.all(aPromises).then((aChanges) => new Promise((resolve, reject) => {
              ...
              if (aChangePromises.length > 0) {
                return Promise.all(aChangePromises).then((
                  aUpdatedChanges,
                ) => {
                  aUpdatedChanges.forEach((oChange) => {
                    aProcessedChanges.push(oChange);
                  });
                  aProcessedChanges.sort((change1, change2) => (
                  ...
                  const oLrepChange = {
                    changes: oResult,
                    componentClassName: "iot.planner.projectsodatav2",
                  };
                  resolve(oLrepChange);
                });
              }
