...
sap.ui.define(
  ...
  (
     ...
     * @version 1.102.1
     */
    const ODataTreeBinding = TreeBinding.extend(
      "sap.ui.model.odata.v2.ODataTreeBinding",
      /** @lends sap.ui.model.odata.v2.ODataTreeBinding.prototype */ {
        constructor(oModel, sPath, oContext, vFilters, mParameters, vSorters) {
          ...
          this.iNumberOfExpandedLevels =
            (mParameters && mParameters.numberOfExpandedLevels) || 0;
          this.iRootLevel = (mParameters && mParameters.rootLevel) || 0;

          this.sCountMode =
            (mParameters && mParameters.countMode) ||
            this.oModel.sDefaultCountMode;
          if (this.sCountMode == CountMode.None) {
            Log.fatal(
              "To use an ODataTreeBinding at least one CountMode must be supported by the service!"
            );
          }
          ...
          this.sOperationMode =
            (mParameters && mParameters.operationMode) ||
          ...
          this.bUseServersideApplicationFilters =
            (mParameters && mParameters.useServersideApplicationFilters) ||
            false;
          this.bUsePreliminaryContext =
            this.mParameters.usePreliminaryContext ||
     ...
     * @returns {string} The filter to use with <code>$filter</code>
     */
    ODataTreeBinding.prototype._getLevelFilterParams = function (
      sOperator,
      iLevel
    ) {
      const oEntityType = this._getEntityType();
     ...
     * @private
     */
    ODataTreeBinding.prototype._loadSingleRootNodeByNavigationProperties =
      function (sNodeId, sRequestKey) {
        const that = this;
        let sGroupId;
        ...
        const sAbsolutePath = this.getResolvedPath();
        if (sAbsolutePath) {
          this.mRequestHandles[sRequestKey] = this.oModel.read(sAbsolutePath, {
            groupId: sGroupId,
            success(oData) {
              const sNavPath = that._getNavPath(that.getPath());

              if (oData) {
                // we expect only one root node
                const oEntry = oData;
                const sKey = that.oModel._getKey(oEntry);
                const oNewContext = that.oModel.getContext(`/${sKey}`);

                that.oRootContext = oNewContext;
                that._processODataObject(
    ...
    ODataTreeBinding.prototype.getRootContexts = function (
      ...
      const sRequestKey = `${sNodeId}-${iStartIndex}-${this._iPageSize}-${iThreshold}`;

      if (this.bHasTreeAnnotations) {
        this.bDisplayRootNode = true;
        // load root level, node id is "null" in this case
        aRootContexts = this._getContextsForNodeId(
          null,
          iStartIndex,
          iLength,
          iThreshold
        ...
        const bIsList = this.oModel.isList(this.sPath, this.getContext());
        if (bIsList) {
          this.bDisplayRootNode = true;
        }
     ...
     * @public
     */
    ODataTreeBinding.prototype.getNodeContexts = function (
      oContext,
      iStartIndex,
      iLength,
      iThreshold
      ...
      if (this.bHasTreeAnnotations) {
        ...
        mRequestParameters.level =
          parseInt(
            oContext.getProperty(this.oTreeProperties["hierarchy-level-for"])
          ) + 1;
      } else {
        const sNavPath = this._getNavPath(oContext.getPath());
      ...
      return this._getContextsForNodeId(
        sNodeId,
        iStartIndex,
        iLength,
        iThreshold,
    ...
    ODataTreeBinding.prototype.hasChildren = function (oContext) {
      ...
      if (this.bHasTreeAnnotations) {
        ...
        const sDrilldownState = oContext.getProperty(
          this.oTreeProperties["hierarchy-drill-state-for"]
        );

        const sNodeKey = this.oModel.getKey(oContext);
        ...
        Log.warning(
          `The entity '${oContext.getPath()}' has not specified Drilldown State property value.`
        );
      ...
      iLength =
        this.oLengths[
          `${oContext.getPath()}/${this._getNavPath(oContext.getPath())}`
        ];
    ...
    ODataTreeBinding.prototype.getChildCount = function (oContext) {
      ...
      if (!oContext) {
        ...
        if (!this.bDisplayRootNode) {
          return this.oLengths[
            `${this.getPath()}/${this._getNavPath(this.getPath())}`
          ];
        }
     ...
     * @private
     */
    ODataTreeBinding.prototype._getContextsForNodeId = function (
      sNodeId,
      iStartIndex,
      iLength,
      iThreshold,
      ...
      if (
        this.oFinalLengths[sNodeId] &&
        this.oLengths[sNodeId] < iStartIndex + iLength
      ) {
      ...
      const fnFindInLoadedSections = function (iStartIndex) {
        // check in the sections which where loaded
        for (let i = 0; i < that._mLoadedSections[sNodeId].length; i++) {
          const oSection = that._mLoadedSections[sNodeId][i];
          // try to find i in the loaded sections. If i is within one of the sections it needs not to be loaded again
          if (
            iStartIndex >= oSection.startIndex &&
            iStartIndex < oSection.startIndex + oSection.length
          ) {
            return true;
          }
        }
      ...
      let i = Math.max(iStartIndex - iThreshold - this._iPageSize, 0);
      if (this.oKeys[sNodeId]) {
        ...
        let iMaxIndexToCheck = iStartIndex + iLength + iThreshold;
        ...
        for (i; i < iMaxIndexToCheck; i++) {
          ...
          if (!sKey) {
            ...
            if (!this.bClientOperation && !fnFindInLoadedSections(i)) {
              aMissingSections = TreeBindingUtils.mergeSections(
                aMissingSections,
        ...
        const iBegin = Math.max(iStartIndex - iThreshold - this._iPageSize, 0);
        const iEnd = iStartIndex + iLength + iThreshold;
        const bExpandThreshold =
          aMissingSections[0] &&
          aMissingSections[0].startIndex === iBegin &&
          aMissingSections[0].startIndex + aMissingSections[0].length === iEnd;

        if (aMissingSections.length > 0 && !bExpandThreshold) {
          // first missing section will be prepended with additional threshold ("negative")
          i = Math.max(
            aMissingSections[0].startIndex - iThreshold - this._iPageSize,
            0
          );
          const iFirstStartIndex = aMissingSections[0].startIndex;
          ...
          i =
            aMissingSections[aMissingSections.length - 1].startIndex +
            aMissingSections[aMissingSections.length - 1].length;
          let iEndIndex = i + iThreshold + this._iPageSize;
        ...
        const iLengthShift = iStartIndex - i;
        aMissingSections = TreeBindingUtils.mergeSections(aMissingSections, {
          startIndex: i,
          length: iLength + iLengthShift + iThreshold,
        });
      }
        ...
        if (aMissingSections.length > 0) {
          ...
          if (this.bHasTreeAnnotations) {
            ...
            if (sNodeId) {
              ...
              const sNodeIdForFilter = oNodeContext.getProperty(
                this.oTreeProperties["hierarchy-node-for"]
              );

              // construct node filter parameter
              const sNodeFilterParameter = this._getNodeFilterParams({
                id: sNodeIdForFilter,
              });
              ...
              if (!this.bClientOperation || this.iRootLevel > 0) {
                const sLevelFilterOperator = this.bClientOperation
                  ? "GE"
                  : "EQ";
                sLevelFilter = this._getLevelFilterParams(
                  sLevelFilterOperator,
                  this.iRootLevel
                );
              }
          ...
          if (!this.bClientOperation) {
            ...
            for (i = 0; i < aMissingSections.length; i++) {
              const oRequestedSection = aMissingSections[i];
              ...
              this._loadSubNodes(
                sNodeId,
                oRequestedSection.startIndex,
                oRequestedSection.length,
                0,
                aParams,
                mRequestParameters,
                oRequestedSection
    ...
    ODataTreeBinding.prototype._getCountForCollection = function () {
      if (
        !this.bHasTreeAnnotations ||
        this.sOperationMode != OperationMode.Auto
      ) {
        Log.error(
          "The Count for the collection can only be retrieved with Hierarchy Annotations and in OperationMode.Auto."
        );
        return;
      }
      ...
      function _handleError(oError) {
        ...
        let sErrorMsg = `Request for $count failed: ${oError.message}`;
        if (oError.response) {
      ...
      if (
        ...
        this.sCountMode == CountMode.InlineRepeat
      ) {
        aParams.push("$top=0");
    ...
    ODataTreeBinding.prototype._getCountForNodeId = function (sNodeId) {
      ...
      let sFilterParams = this.getFilterParams() || "";
      let sNodeFilter = "";
      if (this.bHasTreeAnnotations) {
        // resolve OData-Key to hierarchy node property value for filtering
        const oNodeContext = this.oModel.getContext(`/${sNodeId}`);
        const sHierarchyNodeId = oNodeContext.getProperty(
          this.oTreeProperties["hierarchy-node-for"]
        );
    ...
    ODataTreeBinding.prototype._getParentMap = function (aData) {
      ...
      for (let i = 0; i < aData.length; i++) {
        const sID = aData[i][this.oTreeProperties["hierarchy-node-for"]];
        if (mParentKeys[sID]) {
     ...
     * @private
     */
    ODataTreeBinding.prototype._createKeyMap = function (
      aData,
      bSkipFirstNode
    ) {
      if (aData && aData.length > 0) {
        const mParentsKeys = this._getParentMap(aData);
        const mKeys = {};

        for (let i = bSkipFirstNode ? 1 : 0; i < aData.length; i++) {
          const sParentNodeID =
            aData[i][this.oTreeProperties["hierarchy-parent-node-for"]];
          let sParentKey = mParentsKeys[sParentNodeID];

          if (
            parseInt(aData[i][this.oTreeProperties["hierarchy-level-for"]]) ===
            this.iRootLevel
          ) {
            sParentKey = "null";
          }
    ...
    ODataTreeBinding.prototype._updateNodeKey = function (oNode, sNewKey) {
      const sOldKey = this.oModel.getKey(oNode.context);
      let sParentKey;
      let nIndex;
      if (
        parseInt(
          oNode.context.getProperty(this.oTreeProperties["hierarchy-level-for"])
        ) === this.iRootLevel
      ) {
        sParentKey = "null";
      } else {
    ...
    ODataTreeBinding.prototype._loadSubTree = function (oNode, aParams) {
      return new Promise((resolve, reject) => {
        ...
        if (!this.bHasTreeAnnotations) {
          reject(
            new Error(
              "_loadSubTree: doesn't support hierarchies without tree annotations"
            )
          );
          return;
        }
        ...
        const fnSuccess = function (oData) {
          ...
          if (oData.results.length > 0) {
            const sParentKey = this.oModel.getKey(oData.results[0]);
            this._updateNodeKey(oNode, sParentKey);
            const mKeys = this._createKeyMap(oData.results);
     ...
     * @private
     */
    ODataTreeBinding.prototype._loadSubNodes = function (
      sNodeId,
      iStartIndex,
      iLength,
      iThreshold,
      ...
      const that = this;
      let sGroupId;
      let bInlineCountRequested = false;
        ...
        if (Array.isArray(oData.results) && oData.results.length > 0) {
          ...
          if (that.bHasTreeAnnotations) {
            ...
            for (i = 0; i < oData.results.length; i++) {
              ...
              that.oKeys[sNodeId][mLastNodeIdIndices[sNodeId]] =
                that.oModel._getKey(oEntry);
      ...
      function fnError(oError) {
        ...
        if (oRequestedSection) {
          ...
          let aLoadedSections = [];
          for (let i = 0; i < that._mLoadedSections[sNodeId].length; i++) {
            const oCurrentSection = that._mLoadedSections[sNodeId][i];
            if (
              oRequestedSection.startIndex >= oCurrentSection.startIndex &&
              oRequestedSection.startIndex + oRequestedSection.length <=
                oCurrentSection.startIndex + oCurrentSection.length
            ) {
              // remove the section interval and maintain adapted sections. If start index and length are the same, ignore the section
              if (
                oRequestedSection.startIndex !== oCurrentSection.startIndex &&
                oRequestedSection.length !== oCurrentSection.length
              ) {
                aLoadedSections = TreeBindingUtils.mergeSections(
                  aLoadedSections,
                  {
                ...
                aLoadedSections = TreeBindingUtils.mergeSections(
                  aLoadedSections,
                  {
                    startIndex:
                      oRequestedSection.startIndex + oRequestedSection.length,
                    length:
                      oCurrentSection.startIndex +
                      oCurrentSection.length -
                      (oRequestedSection.startIndex + oRequestedSection.length),
                  }
                );
              }
     ...
     */
    ODataTreeBinding.prototype._loadCompleteTreeWithAnnotations = function (
      aURLParams
    ) {
      const that = this;

      const sRequestKey = ODataTreeBinding.REQUEST_KEY_CLIENT;
      ...
      const fnSuccess = function (oData) {
        ...
        if (oData.results && oData.results.length > 0) {
          ...
          for (let i = 0; i < oData.results.length; i++) {
            ...
            const sParentNodeId =
              oDataObj[that.oTreeProperties["hierarchy-parent-node-for"]];
            let sParentKey = mParentIds[sParentNodeId]; // oDataObj[that.oTreeProperties["hierarchy-parent-node-for"]];

            // the parentNodeID for root nodes (node level == iRootLevel) is "null"
            if (
              parseInt(
                oDataObj[that.oTreeProperties["hierarchy-level-for"]]
              ) === that.iRootLevel
            ) {
              sParentKey = "null";
            }
            ...
            // add the current entry key to the key map, as a child of its parent node
            const sKey = that.oModel._getKey(oDataObj);
            ...
            that._mLoadedSections[sParentKey] =
              that._mLoadedSections[sParentKey] || [];
            that._mLoadedSections[sParentKey][0] = that._mLoadedSections[
              sParentKey
            ][0] || { startIndex: 0, length: 0 };
        ...
        if (
          (that.aApplicationFilters && that.aApplicationFilters.length > 0) ||
      ...
      const fnError = function (oError) {
        ...
        const bAborted = oError.statusCode == 0;
        if (!bAborted) {
          that.oKeys = {};
          that.oLengths = {};
          that.oFinalLengths = {};
     ...
     * @private
     */
    ODataTreeBinding.prototype._refresh = function (
      bForceUpdate,
      mChangedEntities,
      mEntityTypes
    ) {
      ...
      if (!bForceUpdate) {
        if (mEntityTypes) {
          let sResolvedPath = this.getResolvedPath();
          if (sResolvedPath) {
            ...
            const oEntityType =
              this.oModel.oMetadata._getEntityTypeByPath(sResolvedPath);
        ...
        if (!mChangedEntities && !mEntityTypes) {
          // default
          bChangeDetected = true;
        }
      }
     ...
     * @public
     */
    ODataTreeBinding.prototype.filter = function (
      aFilters,
      sFilterType,
      bReturnSuccess
    ) {
      ...
      if (
        sFilterType == FilterType.Control &&
        (!this.bClientOperation || this.sOperationMode == OperationMode.Server)
      ) {
        Log.warning(
          "Filtering with ControlFilters is ONLY possible if the ODataTreeBinding is running in OperationMode.Client or " +
      ...
      if (!this.bInitial) {
        ...
        if (
          ...
          (sFilterType === FilterType.Control ||
            (sFilterType === FilterType.Application &&
              !this.bUseServersideApplicationFilters))
        ) {
          if (this.oAllKeys) {
    ...
    ODataTreeBinding.prototype._applyFilter = function () {
      ...
      if (this.bUseServersideApplicationFilters) {
        ...
        oCombinedFilter = FilterProcessor.combineFilters(
          this.aFilters,
      ...
      const fnFilterKey = function (sKey) {
        const aFiltered = FilterProcessor.apply(
          [sKey],
          oCombinedFilter,
          (vRef, sPath) => {
            const oContext = that.oModel.getContext(`/${vRef}`);
    ...
    ODataTreeBinding.prototype._filterRecursive = function (
      oNode,
      mKeys,
      fnFilterKey
    ) {
      const aChildrenKeys = this.oKeys[oNode.id];

      // node has children
      if (aChildrenKeys) {
        // loop over all children, and search for filter matches depth-first
        oNode.children = oNode.children || [];
        for (let i = 0; i < aChildrenKeys.length; i++) {
          const oChildNode = this._filterRecursive(
            {
              id: aChildrenKeys[i],
            },
            mKeys,
     ...
     * @private
     */
    ODataTreeBinding.prototype.addSortComparators = function (
      aSorters,
      oEntityType
    ) {
      let oPropertyMetadata;
      let sType;

      if (!oEntityType) {
        Log.warning(
          "Cannot determine sort comparators, as entitytype of the collection is unknown!"
        );
        return;
      }
      each(aSorters, (i, oSorter) => {
        if (!oSorter.fnCompare) {
          oPropertyMetadata = this.oModel.oMetadata._getPropertyMetadata(
            oEntityType,
            oSorter.sPath
          );
          ...
          assert(
            oPropertyMetadata,
            `PropertyType for property ${oSorter.sPath} of EntityType ${oEntityType.name} not found!`
          );
     ...
     * @private
     */
    ODataTreeBinding.prototype.checkUpdate = function (
      bForceUpdate,
      mChangedEntities
    ) {
      const sChangeReason = this.sChangeReason
        ? this.sChangeReason
        : ChangeReason.Change;
      ...
      if (!bForceUpdate) {
        if (this.bNeedsUpdate || !mChangedEntities) {
          ...
          each(this.oKeys, (i, aNodeKeys) => {
            each(aNodeKeys, (i, sKey) => {
    ...
    ODataTreeBinding.prototype._getNavPath = function (sPath) {
      ...
      const sAbsolutePath = this.oModel.resolve(sPath, this.getContext());

      if (!sAbsolutePath) {
        return undefined;
      }

      const aPathParts = sAbsolutePath.split("/");
      const sEntityName = aPathParts[aPathParts.length - 1];
      let sNavPath;

      // Only if part contains "(" we are working on a specific entity with children
      const sCurrent = sEntityName.split("(")[0];
     ...
     * @private
     */
    ODataTreeBinding.prototype._processODataObject = function (
      oObject,
      sPath,
      sNavPath
    ) {
      ...
      const oRef = this.oModel._getObject(sPath);
      ...
      if (sNavPath && oObject[sNavPath]) {
        if (Array.isArray(oRef)) {
          oRef.forEach((sRef) => {
            const oObject = that.getModel().getData(`/${sRef}`);
            that._processODataObject(
              oObject,
              `/${sRef}/${sNavPath}`,
              aNavPath.join("/")
    ...
    ODataTreeBinding.prototype._hasTreeAnnotations = function () {
      const { oMetadata } = this.oModel;
      let sAbsolutePath = this.getResolvedPath();
      let oEntityType;
      const sTreeAnnotationNamespace = oMetadata.mNamespaces.sap;
      const that = this;
      ...
      const fnSanityCheckTreeAnnotations = function () {
        let iFoundAnnotations = 0;
        let iMaxAnnotationLength = 0;
        each(that.oTreeProperties, (sPropName, sPropValue) => {
        ...
        if (iFoundAnnotations > 0 && iFoundAnnotations < iMaxAnnotationLength) {
          Log.warning(
      ...
      if (this.mParameters && this.mParameters.treeAnnotationProperties) {
        this.oTreeProperties["hierarchy-level-for"] =
          this.mParameters.treeAnnotationProperties.hierarchyLevelFor;
        this.oTreeProperties["hierarchy-parent-node-for"] =
          this.mParameters.treeAnnotationProperties.hierarchyParentNodeFor;
        this.oTreeProperties["hierarchy-node-for"] =
          this.mParameters.treeAnnotationProperties.hierarchyNodeFor;
        this.oTreeProperties["hierarchy-drill-state-for"] =
          this.mParameters.treeAnnotationProperties.hierarchyDrillStateFor;
      ...
      each(oEntityType.property, (iIndex, oProperty) => {
        if (!oProperty.extensions) {
          return true;
        }
        each(oProperty.extensions, (iIndex, oExtension) => {
          const sName = oExtension.name;
          if (
            oExtension.namespace === sTreeAnnotationNamespace &&
    ...
    ODataTreeBinding.prototype.initialize = function () {
      if (
        ...
        if (this.isResolved()) {
          this._initialize(
            this._fireRefresh.bind(this, { reason: ChangeReason.Refresh })
          );
        } else {
          this._fireRefresh({ reason: ChangeReason.Refresh });
        }
    ...
    ODataTreeBinding.prototype.setContext = function (oContext) {
      ...
      if (Context.hasChanged(this.oContext, oContext)) {
        ...
        if (this.getResolvedPath()) {
          this.resetData();
          this._initialize(
            this._fireChange.bind(this, { reason: ChangeReason.Context })
          );
        } else if (
          !isEmptyObject(this.oAllKeys) ||
          !isEmptyObject(this.oKeys) ||
          !isEmptyObject(this._aNodes)
        ) {
          // binding is now unresolved, but has data
          this.resetData();
          this._fireChange({ reason: ChangeReason.Context });
        }
      }
    ...
    ODataTreeBinding.prototype._applyAdapter = function (fnFireEvent) {
      ...
      let sKeyProperty;
      let sName;
      let sAdapterModuleName = "sap/ui/model/odata/ODataTreeBindingAdapter";
      const sMagnitudeAnnotation = "hierarchy-node-descendant-count-for";
      const sPreorderRankAnnotation = "hierarchy-preorder-rank-for";
      const sSiblingRankAnnotation = "hierarchy-sibling-rank-for";
      const that = this;

      if (!this.bHasTreeAnnotations && !this.oNavigationPaths) {
        Log.error(
          "Neither hierarchy annotations, " +
      ...
      if (this.bHasTreeAnnotations) {
        ...
        each(oEntityType.property, (iIndex, oProperty) => {
          ...
          each(oProperty.extensions, (iIndex, oExtension) => {
            ...
            if (
              oExtension.namespace === that.oModel.oMetadata.mNamespaces.sap &&
              (sName == sMagnitudeAnnotation ||
                sName == sSiblingRankAnnotation ||
        ...
        this.oTreeProperties[sMagnitudeAnnotation] =
          this.oTreeProperties[sMagnitudeAnnotation] ||
        ...
        if (
          ...
          if (this.mParameters.restoreTreeStateAfterChange) {
            if (
              ...
              for (i = oEntityType.key.propertyRef.length - 1; i >= 0; i--) {
                this._aTreeKeyProperties.push(
                  oEntityType.key.propertyRef[i].name
                );
              }
            } else {
              Log.warning(
                "Tree state restoration not possible: Missing annotation " +
          ...
          if (this.mParameters && this.mParameters.select) {
            if (
              this.mParameters.select.indexOf(
                this.oTreeProperties[sMagnitudeAnnotation]
              ) === -1
            ) {
      ...
      sap.ui.require([sAdapterModuleName], (oAdapterModule) => {
    ...
    ODataTreeBinding.prototype._processSelectParameters = function () {
      if (this.mParameters) {
        ...
        if (this.mParameters.select) {
          ...
          const aSelectParams = this.mParameters.select.split(",");
          const aNewSelectParams = [];

          if (this.oNavigationPaths) {
            each(this.oNavigationPaths, (sParamKey, sParamName) => {
          ...
          each(aNewSelectParams, (sParamKey, sParamName) => {
          ...
          if (this.bHasTreeAnnotations) {
            each(this.oTreeProperties, (sAnnotationName, sTreePropName) => {
              if (sTreePropName) {
    ...
    ODataTreeBinding.prototype.getTreeAnnotation = function (sAttributeName) {
      return this.bHasTreeAnnotations
        ? this.oTreeProperties[sAttributeName]
        : undefined;
    };
    ...
    ODataTreeBinding.prototype._getEntityType = function () {
      const sResolvedPath = this.getResolvedPath();
      ...
      if (sResolvedPath) {
        ...
        assert(
          oEntityType,
          `EntityType for path ${sResolvedPath} could not be found!`
        );
        return oEntityType;
      }
    ...
    ODataTreeBinding.prototype.getFilterParams = function () {
      ...
      if (this.aApplicationFilters) {
        this.aApplicationFilters = Array.isArray(this.aApplicationFilters)
          ? this.aApplicationFilters
          : [this.aApplicationFilters];
        if (this.aApplicationFilters.length > 0 && !this.sFilterParams) {
          oGroupedFilter = FilterProcessor.groupFilters(
            this.aApplicationFilters
          );
          this.sFilterParams = ODataUtils._createFilterParams(
            oGroupedFilter,
            this.oModel.oMetadata,
            this.oEntityType
          );
          // Add a bracket around filter params, as they will be combined with tree specific filters
          this.sFilterParams = this.sFilterParams
            ? `(${this.sFilterParams})`
            : "";
        }
      } else {
    ...
    ODataTreeBinding.prototype._abortPendingRequest = function () {
      if (!isEmptyObject(this.mRequestHandles)) {
        ...
        each(this.mRequestHandles, (sRequestKey, oRequestHandle) => {
          if (oRequestHandle) {
